...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
    ...
    <x-gp247::card :title="gp247_language_render($editingId ? 'admin.language.edit' : 'admin.language.add')">
        <form wire:submit="save" class="space-y-4">
            <x-gp247::input :label="gp247_language_render('admin.name')" name="name" wire:model="form.name"
                :error="$errors->first('form.name')" required />

            <x-gp247::input :label="gp247_language_render('admin.code')" name="code" wire:model="form.code"
                :help="gp247_language_render('admin.language.code_help')" :error="$errors->first('form.code')" required />

            <x-gp247::media-input :label="gp247_language_render('admin.icon')" name="icon" type="language"
            ...
            <div class="flex items-center justify-between border-t border-gray-200 pt-4 dark:border-gray-700">
                <x-gp247::button variant="secondary" wire:click="cancelEdit" data-testid="admin-language-form-cancel">{{ gp247_language_render($editingId ? 'admin.cancel' : 'admin.reset') }}</x-gp247::button>
    ...
    <x-gp247::card :title="gp247_language_render('admin.language.list_title')">
        ...
        <x-gp247::table :empty="$rows->isEmpty() ? gp247_language_render('admin.language.no_found') : null">
            <x-slot:head>
                <tr>
                    @foreach (['name' => 'admin.name', 'code' => 'admin.code'] as $f => $labelKey)
                    ...
                    @endforeach
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ gp247_language_render('admin.icon') }}</th>
            ...
            @foreach ($rows as $row)
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 {{ (string) $row->id === (string) $editingId ? 'bg-blue-100 border-l-4 border-blue-500 dark:bg-blue-900 dark:border-blue-500' : '' }}" wire:key="lang-{{ $row->id }}">
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-end gap-1">
                            <x-gp247::button size="sm" variant="ghost" wire:click="editRow('{{ $row->id }}')" data-testid="admin-language-list-edit"><i class="fas fa-edit"></i></x-gp247::button>
                            @unless ($guarded)
