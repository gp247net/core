...
namespace GP247\Core\Database\Seeders;
...
class DataDefaultSeeder extends Seeder
{
    ...
    public $adminEmail = 'admin@gp247.local';
    public $language_default = 'en';
    public $title_en = 'Demo GP247 CMS';
    public $title_vi = 'Demo GP247 CMS';
    ...
    public function run()
    {
        ...
        $db->table(GP247_DB_PREFIX.'admin_menu')->insertOrIgnore($dataMenu);
        
        $dataAdminPermission = $this->dataAdminPermission(GP247_ADMIN_PREFIX);
        $db->table(GP247_DB_PREFIX.'admin_permission')->insertOrIgnore($dataAdminPermission);
        
        $dataAdminRole = $this->dataAdminRole();
        $db->table(GP247_DB_PREFIX.'admin_role')->insertOrIgnore($dataAdminRole);
        
        $dataAdminRolePermission = $this->dataAdminRolePermission();
        $db->table(GP247_DB_PREFIX.'admin_role_permission')->insertOrIgnore($dataAdminRolePermission);

        $dataAdminRoleUser = $this->dataAdminRoleUser();
        $db->table(GP247_DB_PREFIX.'admin_role_user')->insertOrIgnore($dataAdminRoleUser);

        $dataAdminUser = $this->dataAdminUser($this->adminUser, $this->adminPassword, $this->adminEmail);
        $db->table(GP247_DB_PREFIX.'admin_user')->insertOrIgnore($dataAdminUser);

        $dataAdminConfig = $this->dataAdminConfig();
        $db->table(GP247_DB_PREFIX.'admin_config')->insertOrIgnore($dataAdminConfig);

        $dataAdminStore = $this->dataAdminStore($this->adminEmail, $this->language_default, str_replace(['http://','https://'], '', url('/')));
        $db->table(GP247_DB_PREFIX.'admin_store')->insertOrIgnore($dataAdminStore);

        $dataAdminStoreDescription = $this->dataAdminStoreDescription($this->title_en, $this->title_vi);
        $db->table(GP247_DB_PREFIX.'admin_store_description')->insertOrIgnore($dataAdminStoreDescription);

        $dataShopLang = $this->dataShopLang();
        $db->table(GP247_DB_PREFIX.'admin_language')->insertOrIgnore($dataShopLang);

        $db->table(GP247_DB_PREFIX.'admin_home')->insertOrIgnore(
            [
                ['id' => 1,'size' => 12, 'sort'=> 1,'view' => 'gp247-admin::component.home_default','status' => 1],
            ]
        );
    ...
    public function dataMenu() {
        $dataMenu = [
            //Root (level1)
            ['id' => 1,'parent_id' => 0,'sort' => 80,'title' => 'admin.menu_titles.ADMIN_WEBSITE','icon' => 'fas fa-store-alt','uri' => '','key' => 'ADMIN_WEBSITE','type' => 0],
            ['id' => 2,'parent_id' => 0,'sort' => 50,'title' => 'admin.menu_titles.ADMIN_SYSTEM','icon' => 'fas fa-cogs','uri' => '','key' => 'ADMIN_SYSTEM','type' => 0],
            ['id' => 3,'parent_id' => 0,'sort' => 60,'title' => 'admin.menu_titles.ADMIN_EXTENSION','icon' => 'fas fa-th','uri' => '','key' => 'ADMIN_EXTENSION','type' => 0],
            ...
            // id=8 store_maintain removed — merged into store_info (WebsiteInfo)
            ...
            ['id' => 27,'parent_id' => 13,'sort' => 4,'title' => 'admin.menu_titles.server_info','icon' => 'fas fa-solar-panel','uri' => 'admin::server_info','key' => null,'type' => 0],

            //User permission (level3)
    ...
    public function dataAdminPermission($prefix) {
        $dataAdminPermission = [
            ...
            ['id' => '2','name' => 'Store manager','slug' => 'store.full','http_uri' => 'ANY::'.$prefix.'/store_info/*,ANY::'.$prefix.'/store_config/*', 'created_at' => date('Y-m-d H:i:s')],
    ...
    public function dataAdminUser($username, $password, $email) {
        $dataAdminUser = [
            ['id' => 'AU-AAAAA','username' => $username,'password' => $password,'email' => $email,'name' => 'Administrator','avatar' => '/GP247/Core/avatar/admin.png','created_at' => date('Y-m-d H:i:s')]
        ];
    ...
    public function dataAdminConfig() {
        $dataAdminConfig = [
            ...
            ['group' => 'global','code' => 'env_mail','key' => 'email_action_queue','value' => '0','sort' => '1','detail' => 'email.email_action.email_action_queue','store_id' => 0],
    ...
    public function dataAdminStoreDescription($titleE, $titleV) {
        $dataAdminStoreDescription = [
            ['store_id' => GP247_STORE_ID_ROOT,'lang' => 'en','name' => $titleE,'description' => '','keyword' => '','maintain_content' => '<center><img src="/GP247/Core/images/maintenance.jpg" />
            <h3><span style="color:#e74c3c;"><strong>Sorry! We are currently doing site maintenance!</strong></span></h3>
            </center>','maintain_note' => 'Website is in maintenance mode!'],
            ['store_id' => GP247_STORE_ID_ROOT,'lang' => 'vi','name' => $titleV,'description' => '','keyword' => '','maintain_content' => '<center><img src="/GP247/Core/images/maintenance.jpg" />
    ...
    public function dataShopLang() {
        $dataShopLang = [
            ['id' => '1','name' => 'English','code' =>'en','icon' => '/GP247/Core/language/flag_uk.png','status' => '1','rtl' => '0', 'sort' => '1'],
            ['id' => '2','name' => 'Tiếng Việt','code' => 'vi','icon' => '/GP247/Core/language/flag_vn.png','status' => '1','rtl' => '0', 'sort' => '2'],
        ];
        return $dataShopLang;
    }
